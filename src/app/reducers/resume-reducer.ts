...
export function ResumeReducer(state = initialState, action: Action): ResumeReducerState {
  switch (action.type) {
    ...
    case RESUME_LIST_SUCCESS: {
      ...
      const ids = resume.map((res: any) => res._id);
    ...
    case RESUME_DELETE_SKILL: {
      ...
      const resumeId = action.payload.resume_id;
      const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
      oldResume.skills = oldResume.skills.filter((data: any) => data._id !== skill._id);
      const obj = {[resumeId]: oldResume};
      const newEntities = {...state.entities, ...obj};
      return {...state, ...{entities: newEntities}};
    }
    ...
    case LOGOUT_ACTION: {
          ...
          case RESUME_DELETE_EDUCATION: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.education = oldResume.education.filter((data: any) => data._id !== education._id);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          }
          ...
          case RESUME_DELETE_EMPLOYMENT_HISTORY: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.employment_history = oldResume.employment_history.filter((data: any) => data._id !== employmentHistory._id);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          }
          ...
          case RESUME_DELETE_INTEREST: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.interests = oldResume.interests.filter((data: any) => data._id !== interest._id);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          }
          ...
          case RESUME_DELETE_LANGUAGE: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.languages = oldResume.languages.filter((data: any) => data._id !== language._id);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          }
          ...
          case RESUME_DELETE_INDUSTRIAL_EXPOSURE: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.industrialExposures = oldResume.industrialExposures.filter((data: any) => data._id !== industrialExposure._id);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          }
          ...
          case RESUME_UPDATE_AWARDS: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.award_achivements = oldResume.award_achivements.filter((data: any) => data._id !== award._id);
            oldResume.award_achivements.push(award);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          ...
          case RESUME_UPDATE_OBJECTIVE: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.objectives = oldResume.objectives.filter((data: any) => data._id !== objective._id);
            oldResume.objectives.push(objective);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          ...
          case RESUME_UPDATE_REFERENCE: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.refrences = oldResume.refrences.filter((data: any) => data._id !== reference._id);
            oldResume.refrences.push(reference);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          ...
          case RESUME_UPDATE_PROJECT_DETAIL: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.projectDetails = oldResume.projectDetails.filter((data: any) => data._id !== projectDetail._id);
            oldResume.projectDetails.push(projectDetail);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          ...
          case RESUME_UPDATE_STRENGTH: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.strengths = oldResume.strengths.filter((data: any) => data._id !== strength._id);
            oldResume.strengths.push(strength);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
          ...
          case RESUME_UPDATE_WEAKNESS: {
            ...
            const resumeId = action.payload.resume_id;
            const oldResume = JSON.parse(JSON.stringify(state.entities[resumeId]));
            oldResume.weakness = oldResume.weakness.filter((data: any) => data._id !== weakness._id);
            oldResume.weakness.push(weakness);
            const obj = {[resumeId]: oldResume};
            const newEntities = {...state.entities, ...obj};
            return {...state, ...{entities: newEntities}};
